...
public class Grafo<T> {
	...
	public List<T> recorridoTopologico() {
		...
		while (!gradosVertice.isEmpty()) {
			...
			List<Vertice<T>> nodosSinEntradas = gradosVertice.entrySet().stream().filter(x -> x.getValue() == 0)
	...
	private void buscarCaminosAux(Vertice<T> v1, Vertice<T> v2, List<Vertice<T>> marcados,
			List<List<Vertice<T>>> todos) {
