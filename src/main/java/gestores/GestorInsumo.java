...
public class GestorInsumo {
	...
	public ArrayList<Insumo> buscar(String busqueda) {
		...
		return arbol.buscar(busqueda).stream().map(s -> this.getInsumoByStr(s))
	...
	public ArrayList<Insumo> buscarPorCosto(Float busqueda, boolean tipo) {
		...
		if (tipo)
			resultado = arbol.rango((float) 0, busqueda); // buscar por maximo
		else
			resultado = arbol.rango(busqueda, Float.MAX_VALUE); // buscar por minimo
		return resultado.stream().map(r -> this.getInsumoByCosto(r)).collect(Collectors.toCollection(ArrayList::new));
	}
